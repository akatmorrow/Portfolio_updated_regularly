...
def calculate_error(m, b, point):
    x_point = int(point[0])
    y_point = int(point[1])
    difference = y_point - get_y(m, b, x_point)
...
possible_ms_list = list(range(-100, 101))
possible_ms = []
def make_range_complicated(range):
    for point in possible_ms_list:
        possible_ms.append(point * 0.1)
    return possible_ms

possible_ms = make_range_complicated(possible_ms_list)
...
twenty_range = list(range(-20, 21))
possible_bs = make_range_complicated(twenty_range)
#print(possible_bs)
